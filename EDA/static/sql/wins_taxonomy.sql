...
Purpose
-------
Describe how match outcomes occurred and identify invalid, out-of-scope, or
poor-quality observations. Unusual but valid matches (red cards, penalties,
own goals, comebacks, late winners, and results against xG) remain part of the
population and should not be excluded merely for being unusual.

The analysis has two core levels:
    1. A mutually exclusive scoreline category.
    2. Overlapping match-narrative flags.

Both are accompanied by match-circumstance flags and descriptive statistics.
All event and same-match statistic fields are retrospective diagnostics, not
features that can be used to predict the same match.

Objective narrative rules
-------------------------
    - early permanent lead: final, never-relinquished lead by minute 30;
    - late decisive goal: final lead obtained in minute 76 or later;
...
/* Clean up objects so the script can be rerun in the same session. */
...
SELECT
    ...
    COUNT(*) AS matches,
    COUNT(*) FILTER (WHERE n.is_comeback_win) AS comeback_wins,
    COUNT(*) FILTER (WHERE n.is_early_lead_established_and_maintained)
        AS early_permanent_lead_wins,
    COUNT(*) FILTER (WHERE n.has_late_decisive_goal) AS late_wins,
    COUNT(*) FILTER (WHERE n.has_stoppage_time_decisive_goal)
        AS stoppage_time_wins,
    COUNT(*) FILTER (WHERE n.has_multiple_lead_changes)
        AS multiple_lead_change_matches,
    COUNT(*) FILTER (WHERE n.has_red_card) AS red_card_matches,
    COUNT(*) FILTER (WHERE n.has_early_red_card) AS early_red_card_matches,
    COUNT(*) FILTER (WHERE n.has_decisive_penalty) AS decisive_penalty_wins,
    COUNT(*) FILTER (WHERE n.has_decisive_own_goal) AS decisive_own_goal_wins,
    COUNT(*) FILTER (WHERE n.goal_events_match_score) AS reliable_goal_sequences
FROM wins_taxonomy_matches m
...
SELECT
    ...
    m.winner_location,
    COUNT(*) AS matches,
    ROUND(AVG(s.winner_xg_margin)::numeric, 3) AS avg_winner_xg_margin,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY s.winner_xg_margin)
        AS median_winner_xg_margin,
    ROUND(AVG(s.winner_shot_margin)::numeric, 2) AS avg_winner_shot_margin,
    ROUND(AVG(s.winner_shots_on_goal_margin)::numeric, 2)
        AS avg_winner_shots_on_goal_margin,
    ROUND(AVG(s.winner_possession_margin)::numeric, 2)
        AS avg_winner_possession_margin,
    ROUND(AVG(s.winner_shot_conversion)::numeric, 3)
        AS avg_winner_shot_conversion,
    ROUND(AVG(s.winner_goals_minus_xg)::numeric, 3)
        AS avg_winner_goals_minus_xg
FROM wins_taxonomy_matches m
LEFT JOIN wins_taxonomy_statistics s USING (fixture_id)
WHERE m.is_target_eligible AND m.outcome <> 'draw'
...
SELECT
    ...
    m.scoreline_category,
    COUNT(*) AS matches,
    ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER (PARTITION BY m.calendar_year), 2)
        AS pct_matches_in_year,
    ROUND(100.0 * COUNT(*) FILTER (WHERE n.goal_events_match_score)
          / NULLIF(COUNT(*), 0), 2) AS pct_reliable_goal_sequences,
    ROUND(100.0 * COUNT(*) FILTER (WHERE s.has_team_stats_for_both_teams)
          / NULLIF(COUNT(*), 0), 2) AS pct_with_team_stats,
    ROUND(100.0 * COUNT(*) FILTER (WHERE s.has_xg_for_both_teams)
...
SELECT
    ...
    m.status_short,
    m.status_long,
    m.is_target_eligible,
    n.goal_events_match_score,
    n.has_event_data,
    s.has_team_stats_for_both_teams,
    s.has_xg_for_both_teams,
    n.suggested_review_decision
FROM wins_taxonomy_matches m
LEFT JOIN wins_taxonomy_narratives n USING (fixture_id)
LEFT JOIN wins_taxonomy_statistics s USING (fixture_id)
