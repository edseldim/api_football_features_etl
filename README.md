...
| Feature categories | Description | Naming pattern |
| - | - | - |
...
1. **Class imbalance is moderate:** home wins account for approximately 43% of matches, draws for 30%, and away wins for 25–27%. Preserve this natural distribution initially.
2. **Relative strength is the strongest signal:** the away-win rate increases from 18.6% in the weakest relative-strength quintile to 34.9% in the strongest.
3. **Venue-specific recent form is highly relevant:** the away-win rate increases from 20.6% to 33.0% between the lowest and highest away-form quintiles.
4. **One-goal scoreline matches require special attention:** they are the largest and most heterogeneous scoreline group, making them potentially harder to predict and calibrate.
5. **Home advantage is widespread but not team-specific:** approximately 52% of teams are meaningfully stronger at home, while 48% are relatively venue-balanced; both groups are sufficiently represented, so venue-based resampling is unnecessary.
6. **Wider victories show stronger early control:** approximately 41% of two-goal wins and 65% of wins by three or more goals establish and maintain the decisive lead early, compared with about 25% of one-goal wins.

Narrow away wins may be among the hardest outcomes to predict because they follow varied match paths, are often decided late, and can occur without conventional indicators of dominance. This remains an EDA hypothesis that should be confirmed by analyzing model errors and calibration by venue and goal margin.